...
CCMnet_constr <- function(Network_stats, Prob_Distr, Prob_Distr_Params, 
                         ...
                         population, covPattern = NULL, remove_var_last_entry = FALSE, print_info = 0,
                         BayesInference = 0, TranNet = NULL, Ia = NULL, Il = NULL, R_times = NULL,beta_a = NULL,beta_l = NULL
                         ) {
  #P is for an initial starting network
  #TranNet is for the tranmission network for Bayesian inference
  
  if (length(population) == 1) {
    return(uni_modal_constr(Network_stats, Prob_Distr, Prob_Distr_Params, 
                            samplesize, burnin, interval,
                            statsonly, P,
                            population, covPattern, remove_var_last_entry, print_info,
                            BayesInference, TranNet, Ia, Il, R_times, beta_a, beta_l
                            )
           )
...
uni_modal_constr <- function(Network_stats, Prob_Distr, Prob_Distr_Params, 
  ...
  if (error == 0) {
    ...
    BayesInference = BayesInference #Required for Bayesian Inference
    Ia = Ia
    Il = Il
    R_times = R_times
    beta_a = beta_a
