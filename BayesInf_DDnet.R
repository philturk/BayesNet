...
#install.packages("CCMnet_0.0-4.tar.gz", repos = NULL, type = "source")
...
assrtind = switch(bool_Assort + 1, "_noAssrt_", "_assrtNet_")
prior = switch(strong_prior+1, "_weak", "_strong")
result_file = paste("BayesNetInf_",population,prior,"_degdist_", ER_prob,assrtind,runno,".RData", sep="")
...
sample_deg_dist =  TRUE# FALSE #
sample_density = FALSE # TRUE#
sample_dmm = FALSE # TRUE #
num_samples = 100
...
while(valid_graph_start == FALSE) {
  G_start = G.generate(g_type = 2, population = population, bool_ER = bool_ER, bool_Assort = bool_Assort, 
                       bool_Clustering = bool_Clustering, ER_prob = ER_prob, Assort_val = Assort_val, Cluster_val = Cluster_val) ###1/3/17
...
print(paste("G max: ", max(degree(G)/2), ", Sim max: ", max(degree(G_start)/2)))
...
max_init_deg = max(degree(Init_G)/2)
ecount_G = c(network.edgecount(Init_G))
ecount_P = c(network.edgecount(Init_P))
degdist_G = summary(Init_G~degree(0:max_init_deg))
degcor_G = summary(Init_G~degcor)
...
for (mcmc_counter in c(mcmc_counter2:n_mcmc_trials)) {
  ...
  degdist_G = rbind(degdist_G, summary(G~degree(0:max_init_deg)))
  degcor_G = c(degcor_G, summary(G~degcor))
...
save(P_a, G_a, Initial_Data, ecount_G, ecount_P, degdist_G, degcor_G, file = result_file)
