...
# second 23 rows are for .90...etc (20 1+368 rows total (includes header))
new_list <- plyr::adply(sim_dist,1,unlist,.id = NULL)
write.csv(new_list, paste0(file_location, "//sim_dists-comp_dist.csv"))
write.csv(true_dist, paste0(file_location, "//true_dist-comp_dist.csv"))
