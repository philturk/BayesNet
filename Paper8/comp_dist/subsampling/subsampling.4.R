...
sim_dist <- array(NA, dim = c(length(row_names), 23, nsims), 
                  dimnames = list(row_names, 1:23, NULL)) #array to store subsamples
...
write.csv(new_list, paste0(file_location, "//subsamples.4-comp_dist-100.csv"))

# change format, save samples
new_list_samp <- plyr::adply(sim_dist_samp,1,unlist,.id = NULL)
write.csv(new_list_samp, paste0(file_location, "//samples.4-comp_dist-100.csv"))

# change format again, save
df_all_sim = melt_all_props100by100(data = new_list)
write.csv(df_all_sim, paste0(file_location, "//prop40//subsamples100by100.csv"), row.names = FALSE)
