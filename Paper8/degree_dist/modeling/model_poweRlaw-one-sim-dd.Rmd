---
title: "Model - Power Law for One Simulation (degree distribution)"
...
# melt dataframe with all proportions and simulations into one master df_all_sim_dd
df_all_sim_dd = melt_all_props(data = sim_dist_100_dd)
```

## Overview of work in this File

**Copied from fitting component distributions. We are loading data for degree distributions instead.**
...
plot(model_pl, ylab = "CDF", main="Comparing Power Law (red) and Exponential (black), x_min = 5")
...
plot(model_ln, ylab = "CDF", main="Comparing Lognormal (red) and Exponential (black), x_min = 2")
lines(model_exp) #fit looks terrible lol
...
plot(model_ln, ylab = "CDF", main="Comparing Lognormal (red) and Power Law (black), x_min = 2") #note it's just taking the data so doesn't matter if we plot model_ln or model_pl (based on trying both and it looking the same)
